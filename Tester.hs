...
printSuccess funcName input b =
  let strInput =
   ...
   in putStrLn $
        displayColor (if b then Green else Red) $
...
displayColor color str =
  let -- https://en.wikipedia.org/wiki/ANSI_escape_code
...
tests_decode theCodes =
  map
    (uncurry $ Test "decode theCodes" $ decode theCodes)
...
tests_encode theCodes =
  map
    (uncurry $ Test "encode theCodes" $ encode theCodes)
    -- Tests from previous assignment's starter code...
    [ ( "Haskell!",
